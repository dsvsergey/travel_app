...
class _CustomCalendarViewerState extends State<CustomCalendarViewer>
      ...
      setState(() {
        if ((widget.calendarType == CustomCalendarType.monthsAndYears &&
          ...
          if (widget.animateDirection ==
                  CustomCalendarAnimatedDirection.horizontal ||
              widget.calendarType == CustomCalendarType.viewFullYear) {
            triggerAnimation(toRight: false);
          } else {
                          ...
                          itemBuilder: (_, idx) {
                            if (count == 0) {
                              ...
                              return InkWell(
                                ...
                                onTap: () {
                                  setState(() {
                                    firstDay = DateFormat('E').format(DateTime(
                                        currentDate.year, index + 1, 1));
                                    getExtraDays();
      ...
      if (addRange == 0) {
        ...
        if (firstRangeDate!.date != date) {
          ranges!.add(RangeDate(
            ...
            startColor: widget.calendarType == CustomCalendarType.multiRanges
                ? widget.activeColor
                : addRangeColor,
            endColor: widget.calendarType == CustomCalendarType.multiRanges
                ? widget.activeColor
                : addRangeColor,
            textColor: widget.calendarType == CustomCalendarType.multiRanges
                ? widget.activeStyle.color
  ...
  List checkInRange(DateTime date) {
    for (int i = 0; i < ranges!.length; i++) {
      ...
      if (start == end) {
        dates!.add(Date(
            date: start,
            color: range.startColor ?? range.endColor,
        ...
        if (start.isAfter(end)) {
          ...
          Color? colorSwitcher = range.startColor;
          range.startColor = range.endColor;
          range.endColor = colorSwitcher;
        }
  ...
  void showOrHideOverlay(
    ...
    setState(() {
      ...
      if (inRange[0] != -1) {
        if (ranges![inRange[0]].toolTipEnMessage.isNotEmpty) {
          haveMessage = true;
        }
      }
      if (dateIndex != -1) {
        if (dates![dateIndex].toolTipEnMessage.isNotEmpty) {
          haveMessage = true;
        }
      }
      if (haveMessage ||
        ...
        child: Stack(
          children: [
            Container(
              alignment: Alignment.center,
              margin: inRange[0] == -1
                  ? edge(padding: const EdgeInsets.all(2))
                  : edge(padding: const EdgeInsets.all(1)),
                    ...
                    Container(
                      decoration: BoxDecoration(
                        ...
                        color: inRange[0] == -1
                            ? (dateIndex != -1
                                ? (dates == null ||
                                        dates![dateIndex].color == null)
                                    ? widget.activeColor
                                    : dates![dateIndex].color
                                : Colors.transparent)
                            : (ranges == null ||
                                    ranges![inRange[0]].startColor == null)
                                ? widget.activeColor
                                : ranges![inRange[0]].startColor,
                      ),
                    ),
  ...
  Widget buildYearsCard() {
    return AnimatedContainer(
      ...
      margin: edge(
          padding: EdgeInsets.only(
        left: widget.headerMargin.left + 70,
...
class DiagonalPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    paint.shader = LinearGradient(
      begin: isStart ? Alignment.topLeft : Alignment.bottomRight,
      end: isStart ? Alignment.bottomRight : Alignment.topLeft,
